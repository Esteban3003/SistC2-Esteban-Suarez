...
d_c = [.0001 .00009 700000 .0001];        %Corriente, velocidad , Ángulo, psita
Q_c = diag(d_c);                           %Q actua sobre las VARIABLES DE ESTADO
R_c = 1;                                 %R actua sobre la ACCION DE CONTROL
...
dead_zone = 3;                         %Zona muerta máxima del Actuador
...
%Vector perturbación
Tl = 0*(-1)*max(Tl_E)*square(t*2*pi/5);
Tl(Tl<0)=0;
...
for i=1: N
    ...
    % La planta recibe u_real, que es u_raw afectado por la zona muerta
    ...
    % x_obs = A_d*x_obs + B_d*u_ideal(i) + K_o*(y - y_obs);
    x_obs = A_d*x_obs + B_d*u_real(i);  % sin corrección
...
plot(t ,u_real); title('Acción de control real V [V]');grid on;hold on;
subplot(3,2,6);hold on;
plot(t ,u_ideal); title('Acción de control ideal V [V]');grid on;hold on;
